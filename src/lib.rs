    ...
    loop {
        ...
        process
            .until_closes(async {
                ...
                //let game_instance = process.read_pointer(main_module_base.add(0x4A2DA88), asr::PointerSize::Bit64).unwrap_or(Address::NULL).add(0x20);
                ...
                loop {
                    ...
                    let scenario_progress = scenario_progress_watcher.update_infallible(process.read(chapter_data_addr.add(0x70)).unwrap_or_default()); //scenario_progress_pointer.update_value(&process);
                    ...
                    let battle_id = encounter_watcher.update_infallible(
                        module.get_fname::<32>(&process, process.read(battle_layout_row_ptr.deref_offsets(&process,&module).unwrap_or(Address::NULL).add(0x8)).unwrap_or(FNameKey::zeroed()))
                    ...
                    if chapter.current == Chapter::ImperialChina as i8 {
                            ...
                            && scenario_progress.current < 531 //only run the following checks if you are in the gauntlet (and before Yi Pei Kou)
                        {
                            ...
                            if duration_frames.current == 180
                                && (duration_frames.old == 0 || frame_pointer.current < frame_pointer.old)
                            {
                        ...
                        
                    {
                        ...
                        {
                            if battle_id.old != 256 //count starts and resets on entering Odio fight
                            {
                                bosses_defeated_watcher.update_infallible(0);
                            }
                            if duration_frames.current == 180
                        ...
                        else if battle_id.current == 88 && settings.dominion_jaggedy_jacks  //Jaggedy Jacks
                        {
                            if battle_id.old != 88 //count starts and resets on entering Jaggedy Jacks fight
                            {
                                bosses_defeated_watcher.update_infallible(0);
                            }
                            if duration_frames.current == 180
